using BlazorMauiObjectDetectionML.YoloParser;
using BlazorMauiObjectDetectionML.DataStructures;
...
namespace BlazorMauiObjectDetectionML.Services
{
    ...
    public class VideoCaptureService
    {
        ...
        void DrawBoundingBox(IList<YoloBoundingBox> filteredBoundingBoxes, SKCanvas canvas)
        {
            ...
            foreach (var box in filteredBoundingBoxes)
            {
                ...
                {
                    Color = System.Drawing.Color.Black.ToSKColor(),
        ...
        public IList<YoloBoundingBox> DetectObjects(MLImage bitmap)
        {
            ...
            while (!token.IsCancellationRequested)
            {
                if (FilesToProcess.TryDequeue(out var bitmap))
                {
                    ...
                    if ((DateTime.Now - LastBoxDetection).TotalMilliseconds > 250)
                    {
                        ...
                        var imageML = MLImage.CreateFromStream(stream);
    
                        LastYoloBoundingBoxes = DetectObjects(imageML);
        ...
        public async Task ExecuteAsync(CancellationToken cancellationToken)
        {
            ...
            using (var mediaPlayer = new LibVLCSharp.Shared.MediaPlayer(libvlc))
            {
                ...
                // Create new media
                var path = Path.Combine(AppContext.BaseDirectory, @"assets\videos\Street_traffic.mp4");
